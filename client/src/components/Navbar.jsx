    ...
    <header className='w-full bg-transparent fixed top-0 left-0 right-0 transition-all ease-in duration-300'>
        <nav className={`py-4 lg:px-24 px-24 ${isSticky ? "sticky top-0 left-0 right-0 bg-blue-300" : ""}`}>
            <div className='flex justify-between items-center text-base gap-8'>
                ...
                <Link to="/" className="text-2xl font-bold  text-blue-700  flex items-center gap-2j" ><FaBlog className='inline-block'/>Book</Link>
            ...
            <div className={`space-y-4 px-4 mt-16 py-7 bg-blue-700 ${isMenuOpen ? "black fixed top-0 right-0 left-0":"hidden"}`}>
                {
                    navItems.map(({link, path}) => (
